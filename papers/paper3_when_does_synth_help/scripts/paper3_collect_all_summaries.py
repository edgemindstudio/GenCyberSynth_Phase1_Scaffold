...
def main():
    ...
    arts = args.artifacts
    models = args.models or [p.name for p in arts.iterdir() if p.is_dir()]

    rows: List[Dict[str, Any]] = []

    for m in sorted(models):
        summ_dir = arts / m / "summaries"
        if not summ_dir.exists():
            continue

        for sp in sorted(summ_dir.glob("summary_*.json")):
            s = load_json(sp)
    ...
    if not rows:
        print("[warn] no summary_*.json files found.")
        return
